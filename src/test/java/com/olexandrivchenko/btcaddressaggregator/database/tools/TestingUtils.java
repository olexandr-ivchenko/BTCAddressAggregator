...
public class TestingUtils {
    ...
    public static Block loadBlock(String blockNumber) throws IOException {
        try {
            String jsonBody = IOUtils.toString(
                    TestingUtils.class.getResourceAsStream("/com/olexandrivchenko/btcaddressaggregator/database/blocks/" + blockNumber + ".json"),
                    "UTF-8");
    ...
    public static Tx loadTransaction(String txid) throws IOException {
        try {
            String jsonBody = IOUtils.toString(
                    TestingUtils.class.getResourceAsStream("/com/olexandrivchenko/btcaddressaggregator/database/transactions/" + txid + ".json"),
                    "UTF-8");
            ObjectMapper mapper = new ObjectMapper();
